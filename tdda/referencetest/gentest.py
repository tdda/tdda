...
EURO_STR_DATE_TERM = r'(\d{1,2})\s' + MONTH_TERM + r'(\d{2,4})'
US_STR_DATE_TERM = MONTH_TERM + r'(\d{1,2})\,?\s?' + r'(\d{2,4})'
TIME_TERM = r'(\d{1,2}):(\d{1,2})(:\d{1,2}|)(\.?\d*)'
TZ_TERM = r' ?([+\-]\d{2}:?\d{2})?\]?Z?'
TS_TERM = '[ T]'
DS = '.*'
ND = r'(|.*[^\d])'
NUM_DATETIME_RE = re.compile(ND + NUM_DATE_TERM + TS_TERM + TIME_TERM + TZ_TERM + DS)
NUM_DATE_RE = re.compile(ND + NUM_DATE_TERM + DS)
EURO_STR_DATETIME_RE = re.compile(
    ND + EURO_STR_DATE_TERM + TS_TERM + TIME_TERM + TZ_TERM + DS
)
EURO_STR_DATE_RE = re.compile(ND + EURO_STR_DATE_TERM + DS)
US_STR_DATETIME_RE = re.compile(
    ND + US_STR_DATE_TERM + TS_TERM + TIME_TERM + TZ_TERM + DS
)
...
class TestGenerator:
    ...
    def generate_exclusions(self):
        ...
        if self.iterations < 2:  # though could still do specifics...
            return
        ref_files = [
            r for r in os.listdir(self.refdir) if not os.path.isdir(self.ref_path(r))
        ]
    ...
    def update_exclusions_with_specifics(self, name, exc, debug=False):
        ...
        for k in ('host', 'ip', 'cwd', 'user', 'homedir', 'tmpdir'):
            ...
            if any(
                ...
                if k == 'homedir':
                    warning = (
                        '*** WARNING: Non-portable reference to '
                        "user's home dir (%s) found in %s" % (specific_string, name)
                    )
        ...
        specific_date_lines = [
            s for s in specifics.values() if s.datelike
        ]  # and not s.rex_inputs
        # and not s.remove
        # and not s.substring]
        extradates = self.find_specific_dates(specific_date_lines)
        specific_dt_lines = [
            s for s in specifics.values() if s.dtlike
        ]  # and not s.rex_inputs
        # and not s.remove
        # and not s.substring]
    ...
    def check_for_specific_references(self, path, filetype):
        ...
        if lines:
            for i, line in enumerate(lines, 1):
                ...
                user = self.user in line and (not (homedir and self.user_in_home))
                if any((datelike, dtlike, host, ip, cwd, homedir, tmpdir, user)):
                    specifics[i] = Specifics(
                        line,
                        host,
                        ip,
                        cwd,
    ...
    def snapshot_fail(self):
        ...
        if len(self.snapshot) > self.max_snapshot_files:
            print(
                '*** Too many files in reference directories (max %d).'
                % len(self.snapshot),
                file=sys.stderr,
            )
            print(
    ...
    def fail_if_exception(self, exc):
        if exc:
            print(
                '***ERROR: Exception occurred running command.\n%s.' % str(exc),
                sys.stderr,
            )
            sys.exit(1)
    ...
    def fail_if_bad_exit_code(self, r):
        if r.exit_code != 0 and self.require_zero_exit_code:
            print(
                '*** Non-zero exit code of %d generated by command.' % r.exit_code,
                file=sys.stderr,
            )
            if r.err:
    ...
    def copy_reference_stream_output(self, run):
        ...
        if self.check_stdout:
            ...
            print(
                'Saved (%sempty) output to stdout to %s.'
                % (
        ...
        if self.check_stderr:
            ...
            print(
                'Saved (%sempty) output to stderr to %s.'
                % (
    ...
    def create_or_empty_ref_dir(self):
        ...
        """
        if os.path.exists(self.refdir):
            paths = [os.path.join(self.refdir, f) for f in os.listdir(self.refdir)]
            for path in paths:
    ...
    def snapshot_filesystem(self):
        ...
        dirs = [
            d
            for d in self.reference_files[1]
    ...
    def add_globs(self, run):
        ...
        for path in reference_files:
            if '?' in path or '*' in path:
                ...
                if not matches:
                    print(
                        "*** Warning: Pattern '%s' matched no files; ignoring." % path
                    )
                else:
    ...
    def ignore(self, name):
        if os.path.isdir(name) and name.startswith(self.refdir):
            return True
        return name == '__pycache__' or name.endswith('.pyc') or name == '.DSStore'
    ...
    def write_script(self):
        ...
        if self.tmpdir_used:
            set_tmpdir = '\n    '.join(
                [
                    ...
                    "    os.environ['%s'] = tmpdir" % self.tmp_dir_shell_var,
        ...
        with open(self.script, 'w') as f:
            ...
            if self.check_stdout:
                path = as_join_repr(self.stdout_path(), self.cwd, self.ref_subdir())
                exc = self.exclusions.get('STDOUT')
                (patterns, removals, substrings) = exc if exc else (None, None, None)
                f.write(
                    test_def(
                        'stdout',
                        'self.output',
            ...
            if self.check_stderr:
                path = as_join_repr(self.stderr_path(), self.cwd, self.ref_subdir())
            ...
            for path in reference_files:
                ...
                actual_path = as_join_repr(
                    path, self.cwd, self.ref_subdir(), inc_tmpdir=True
                )
                exc = self.exclusions.get(short_path)
                filetype = self.filetypes.get(short_path) or FileType(short_path)
                if filetype.text:
                    if exc:
                    ...
                    f.write(test_def(testname, actual_path, 'BinaryFile', ref_path))
    ...
    def generated_file_paths(self, in_cls=False):
        ...
        """
        return [
            self.path_repr(ref_path, as_dollar=False, in_cls=in_cls)
    ...
    def cli_command(self, zec=None):
        ...
        files += ' '.join(repr(f) for f in self.raw_files if not f in exclusions)
        flags = []
    ...
    def summary(self, inc_timings=True):
        ...
        n_tests = (
            len(reference_files) + int(self.check_stdout) + int(self.check_stderr) + 2
        )  # exit code and no error
        if inc_timings:
            lines = ['Command execution took: %s' % format_time(r.duration)]
        lines += (
            [
                '',
                'SUMMARY:',
                '',
                'Directory to run in:        %s'
                % ('.' if self.relative_paths else self.cwd),
                'Shell command:              %s' % self.command,
                'Test script generated:      %s' % self.script,
                'Reference files:%s' % ('' if reference_files else ' (none)'),
            ]
            ...
            + [
                ...
                'Clobbering permitted:       %s' % ('no' if self.no_clobber else 'yes'),
    ...
    def abs_or_rel(self, path):
        ...
        """
        return as_join_repr(path, self.cwd, as_pwd='.') if self.relative_paths else path
    ...
    def path_repr(self, path, as_dollar=True, in_cls=False):
        ...
        if self.tmp_dir_shell_var:
            if as_dollar:
                ...
                return as_join_repr(path, self.cwd, inc_tmpdir=True, in_cls=in_cls)
        else:
            if as_dollar:
                return as_join_repr(path, self.cwd, as_pwd='$(pwd)', in_cls=in_cls)
            else:
...
def canonicalize(path, default_ext=None, reject_other_exts=True):
    ...
    if default_ext is not None:
        ...
        if reject_other_exts and ext and ext != default_ext:
            print('\n*** Extension %s on %s must be %s' % (ext, path, default_ext))
            sys.exit(1)
...
def test_def(
    ...
    if substrings:
        ...
        ss = ['self.orig_tmpdir' if s == TMPDIR else repr(s) for s in substrings]
...
def get_int(msg, default, minval=None, maxval=None):
    ...
    while not done:
        ...
        try:
            n = int(reply)
            if (minval is None or n >= minval) and (maxval is None or n <= maxval):
                done = True
        except:
            pass
    return n
...
def wizard(iterations):
    ...
    no_clobber = not (yes_no('Clobber (overwrite) previous outputs (if they exist)'))
    iterations = get_int('Number of times to run script', iterations, 1, None)
    return (
        shellcommand,
        output_script,
        reference_files,
...
def gentest_parser(usage=''):
    ...
    parser.add_argument('-m', '--max-files', type=int, help='Max files to track')
    parser.add_argument(
        '-r',
        '--relative-paths',
        action='store_true',
    ...
    parser.add_argument(
        ...
        '--no-stdout',
        action='store_true',
        help='Do not generate a test checking output to STDOUT',
    )
    parser.add_argument(
        '-E',
        '--no-stderr',
        action='store_true',
        help='Do not generate a test checking output to STDERR',
    )
    parser.add_argument(
        '-Z',
        '--non-zero-exit',
