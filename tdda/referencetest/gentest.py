...
def gentest(shellcommand=None, output_script=None, *reference_files):
    ...
    files = [canonicalize(f) for f in reference_files
             if f.lower() not in ('stdout', 'stderr', 'nonzeroexit')]
...
class TestGenerator:
    def __init__(self, cwd, command, script, reference_files, check_stdout,
                 check_stderr=True, require_zero_exit_code=True):
        self.cwd = cwd
        ...
        print('\nRunning command %s to generate output.\n'
              % repr(self.command))
        out, err, exc, code = exec_command(self.command, self.cwd)
        self.exit_code = code
        ...
        if exc is not None:
            ...
            sys.exit(1)
        if code != 0:
            print('*** Non-zero exit code of %d generated by command.' % code)
            if self.require_zero_exit_code:
                print('    Add NONZEROEXIT to files list to allow this.')
                sys.exit(1)
            else:
                print('    This will be used as expected exit code in tests.'
                      '\n')
        if err:
...
def wizard():
    ...
    require_zero_exit_code = yes_no('Require zero exit code')
