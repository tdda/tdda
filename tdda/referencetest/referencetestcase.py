...
If the tests are run with the ``--tagged`` (or ``-1``) command-line option,
then only tests that have been decorated with ``referencetestcase.tag``,
are run. This is a mechanism for allowing only a chosen subset of tests to
be run, which is useful during development. The ``@tag`` decorator can be
applied to either test classes or test methods.

If the tests are run with the ``--istagged`` (or ``-0``) command-line
option, then no tests are run; instead, the framework reports the full
module names of any test classes that have been decorated with ``@tag``.
