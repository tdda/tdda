...
try:
    ...
    print(
        'Skipping Postgres tests (no driver library (pip install pygresql)',
        file=sys.stderr,
    )
    pgdb = None
...
try:
    ...
    print(
        'Skipping SQLite tests (no driver library for sqlite3)',
        file=sys.stderr,
    )
    sqlite3 = None
...
if pgdb and not os.path.exists(POSTGRES_CONN_FILE):
    print(
        'Skipping Postgres because no connection file exists at %s.'
        % POSTGRES_CONN_FILE
    )
...
class TestDatabaseHandlers:
    ...
    def test_handler_unique_values(self):
        elements = self.dbh.resolve_table('elements')
        self.assertEqual(
            self.dbh.get_database_unique_values(elements, 'ChemicalSeries'),
            [
                'Actinoid',
                'Alkali metal',
...
class TestDatabaseConstraintDiscoverers:
    ...
    def test_discover_elements(self):
        ...
        elements = self.dbh.resolve_table('elements')
        constraints = discover_db_table(
            self.dbh.dbtype, self.db, elements, inc_rex=False
        )
    ...
    def test_discover_elements_rex(self):
        ...
        elements = self.dbh.resolve_table('elements')
        constraints = discover_db_table(
            self.dbh.dbtype, self.db, elements, inc_rex=True, seed=827364
        )
        constraints.remove_field('_rowindex')  # hidden field, ignore it
...
class TestDatabaseConstraintVerifiers:
    ...
    def test_verify_elements(self):
        ...
        elements = self.dbh.resolve_table('elements')
        result = verify_db_table(
            self.dbh.dbtype, self.db, elements, constraints_file, testing=True
        )
...
@unittest.skipIf(
    pgdb is None or not os.path.exists(POSTGRES_CONN_FILE),
