...
GEN_PY = (
    'Generate Python with .py target and --to pl.r to see required post-processing.'
)
...
def serial_to_polars_read_csv_args_and_postproc(
    ...
    if isinstance(md.fields, list):  # full schema
        ...
        if has_unknowns:
            ...
            rename_map = {
                fld.csvname: fld.name for fld in md.fields if fld.csvname != fld.name
            }
        else:
    ...
    for field, fmd in fields.items():
        ...
        if fmd.fieldtype and fmd.fieldtype.lower().startswith('bool'):
            ...
            bads = ', '.join(
                v for v in (trues + falses) if v.lower() not in ('true', 'false')
            )
            if bads:
                schema[fmd.csvname] = f(pl.String)
