...
Those can be installed using the script `00-install.R`.
...
* This generates a test for running the script
  ...
  This script computes weighted average tolerances for three taxa
...
* You should now have a directory `ref/one`, which will contain two files.
  STDOUT will contain the numeric output (two lines).
  STDERR will be empty for this script.
...
* At the first prompt, enter the command to run the second script:

    Enter shell command to be tested: Rscript 2-compute-cumulative-percentiles.R

* At the second prompt, enter a shorter name for the test script:
...
or you may get something more like:
    ...
    Traceback (most recent call last):
      File "/home/tdda/tmp/gentest_examples/r-examples/test_two.py", line 52, in test_plots2_pdf
        self.assertTextFileCorrect(os.path.join(self.cwd, 'plots2.pdf'),
...
generates for the PDF file looks something like this:
